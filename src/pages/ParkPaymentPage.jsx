...
import {
    ChevronLeft, ChevronRight, Clock, Info, ChevronDown,
...
const ParkPaymentPage = () => {
    ...
    return (
        <div className="min-h-screen bg-[#F5F5FA] dark:bg-[#0f1115] transition-colors duration-300 pb-12 lg:pb-24 overflow-x-hidden">
            ...
            <div className="bg-white dark:bg-[#16181d] border-b border-gray-100 dark:border-gray-800 sticky top-0 z-30">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center gap-4 sm:gap-6">
                    <button
                        onClick={handleBack}
                    ...
                    <div>
                        <h1 className="text-lg sm:text-xl font-black text-gray-900 dark:text-white leading-none mb-1">Confirm & Pay</h1>
                        <p className="text-[9px] sm:text-[10px] uppercase font-bold text-gray-400 tracking-widest">{park?.name}</p>
                    </div>
                </div>
            </div>

            {/* Timer Header */}
            <div className="bg-amber-50/80 dark:bg-amber-900/10 border-b border-amber-100 dark:border-amber-900/20 py-2.5 sm:py-3 flex items-center justify-center gap-2 sm:gap-3 animate-pulse">
                <Clock className="w-4 h-4 sm:w-5 sm:h-5 text-amber-500" />
                <p className="text-xs sm:text-sm font-black text-amber-700 dark:text-amber-400">Seats reserved for <span className="font-mono text-sm sm:text-base">{formatTime(timeLeft)}</span></p>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 md:py-12">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
                    ...
                    <div className="lg:col-span-2 space-y-6 md:space-y-8">
                        ...
                        {/* Contact Details */}
                        <section className="bg-white dark:bg-[#16181d] rounded-2xl md:rounded-[32px] p-5 md:p-8 border border-gray-100 dark:border-gray-800 shadow-sm transition-all">
                            <h2 className="text-lg md:text-xl font-black text-gray-900 dark:text-white mb-6 md:mb-8">Contact Details</h2>
                            <div className="space-y-6">
                                <div>
                                    <label className="text-[10px] uppercase font-black text-gray-400 tracking-widest block mb-2 sm:mb-3">Mobile Number *</label>
                                    <input
                                        type="tel"
                                        value={phone}
                        ...
                        {/* Coupons Section */}
                        <section className="bg-white dark:bg-[#16181d] rounded-2xl md:rounded-[32px] p-5 md:p-8 border border-gray-100 dark:border-gray-800 shadow-sm transition-all">
                            <h2 className="text-lg md:text-xl font-black text-gray-900 dark:text-white mb-6 md:mb-8">Apply coupons</h2>
                            <div className="flex gap-2 sm:gap-4 mb-6">
                                <input
                                    ...
                                    className="flex-1 min-w-0 bg-[#F5F5FA] dark:bg-gray-800/50 border border-transparent focus:border-primary/30 rounded-2xl px-4 sm:px-6 py-3.5 sm:py-4 outline-none text-sm font-medium dark:text-white transition-all"
                                />
                                <button className="px-4 sm:px-8 py-3.5 sm:py-4 bg-primary/10 text-primary text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-primary hover:text-white transition-all shrink-0">Apply</button>
                            </div>

                            <div className="border-t border-gray-50 dark:border-gray-800 pt-5 sm:pt-6">
                                <button
                                ...
                                {isCouponOpen && (
                                    <div className="mt-4 sm:mt-6">
                                        <div className="p-4 sm:p-6 border-2 border-dashed border-gray-100 dark:border-gray-800 rounded-2xl sm:rounded-3xl flex items-center justify-between group hover:border-primary/20 transition-all cursor-pointer">
                                            <div className="space-y-1 min-w-0">
                                                <p className="text-sm font-black text-gray-700 dark:text-gray-300">WELCOME50</p>
                                                <p className="text-xs font-bold text-gray-400 truncate">Save ₹50 on your first booking</p>
                                            </div>
                                            <button className="text-[10px] font-black text-primary uppercase tracking-widest shrink-0 ml-2">Apply</button>
                                        </div>
                                    </div>
                                )}
                            </div>
                        ...
                        {/* Payment Methods */}
                        <section className="bg-white dark:bg-[#16181d] rounded-2xl md:rounded-[32px] p-5 md:p-8 border border-gray-100 dark:border-gray-800 shadow-sm transition-all">
                            <h2 className="text-lg md:text-xl font-black text-gray-900 dark:text-white mb-6 md:mb-8">Choose Payment Method</h2>

                            {/* UPI Section */}
                            <div className="bg-[#F5F5FA] dark:bg-gray-800/20 rounded-2xl md:rounded-[24px] p-4 sm:p-6 md:p-8 mb-6 border border-gray-50 dark:border-gray-800">
                            ...
                            <div className="relative flex items-center justify-center mb-6 sm:mb-8">
                                ...
                                <span className="relative bg-white dark:bg-[#16181d] px-4 sm:px-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">Or pay with</span>
                            </div>
                            ...
                            <div className="space-y-4">
                                {[
                                    ...
                                    <div
                                        ...
                                        className={`bg-white dark:bg-transparent rounded-2xl sm:rounded-3xl p-4 sm:p-6 border flex items-center justify-between cursor-pointer transition-all group ${selectedMethod === method.id ? 'border-primary bg-primary/5 dark:bg-primary/5 shadow-md ring-1 ring-primary/10' : 'border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/30'
                                            }`}
                                    >
                                        <div className="flex items-center gap-3 sm:gap-5 min-w-0">
                                            <div className={`p-2.5 sm:p-3 rounded-xl sm:rounded-2xl transition-colors shrink-0 ${selectedMethod === method.id ? 'bg-primary/20' : 'bg-gray-50 dark:bg-gray-800 group-hover:bg-primary/10'}`}>
                                                <method.icon className={`w-5 h-5 sm:w-6 sm:h-6 transition-colors ${selectedMethod === method.id ? 'text-primary' : 'text-gray-400 group-hover:text-primary'}`} />
                                            </div>
                                            <div className="min-w-0">
                                                <h4 className="text-xs sm:text-sm font-black text-gray-800 dark:text-gray-200 mb-0.5 sm:mb-1 truncate">{method.name}</h4>
                                                <p className="text-[10px] sm:text-xs font-bold text-gray-400 truncate">{method.subtitle}</p>
                                            </div>
                                        </div>
                                        <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center transition-all shrink-0 ${selectedMethod === method.id ? 'bg-primary' : 'bg-gray-50 dark:bg-gray-800 group-hover:bg-primary group-hover:translate-x-1'}`}>
                                            <ChevronLeft className={`w-4 h-4 sm:w-5 sm:h-5 rotate-180 transition-colors ${selectedMethod === method.id ? 'text-white' : 'text-gray-300 group-hover:text-white'}`} />
                                        </div>
                                    </div>
                                ))}
                            </div>
